...
export default function ResetPasswordPage(){
  ...
  const passError = !password
    ...
    : (/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).{8,}$/.test(password) ? '' : 'Mín. 8 con mayúscula, minúscula y número')
  ...
  async function onSubmit(e){
    e.preventDefault()
    setTouchedPass(true); setTouchedConfirm(true)
    if (!isValid) return
    setSubmitting(true)
    setError('')
    try{
      ...
      toast({ title: 'Contraseña actualizada', status: 'success', duration: 3000 })
    ...
    <Flex minH="100vh" bg="brand.50" align="center" justify="center">
      <Box className="auth-container">
        <Box className="auth-card">
          <Heading size="lg" mb={5}>Restablecer contraseña</Heading>
          {!token && <Text mb={4} color="red.500">Token inválido o faltante.</Text>}
          <form onSubmit={onSubmit}>
            <FormControl mb={5} isInvalid={touchedPass && !!passError}>
              <FormLabel>Nueva contraseña</FormLabel>
              <Input type="password" value={password} onChange={e=>setPassword(e.target.value)} onBlur={()=>setTouchedPass(true)} placeholder="Nueva contraseña" borderRadius="full" />
              <FormErrorMessage>{passError}</FormErrorMessage>
            </FormControl>
            <FormControl mb={5} isInvalid={touchedConfirm && !!confirmError}>
              <FormLabel>Confirmar contraseña</FormLabel>
              <Input type="password" value={confirm} onChange={e=>setConfirm(e.target.value)} onBlur={()=>setTouchedConfirm(true)} placeholder="Repite contraseña" borderRadius="full" />
              <FormErrorMessage>{confirmError}</FormErrorMessage>
            </FormControl>
            {error && <Text color="red.500" mb={4}>{error}</Text>}
            <Button type="submit" isLoading={submitting} isDisabled={!isValid || submitting} w="full" size="lg">Restablecer</Button>
          </form>
